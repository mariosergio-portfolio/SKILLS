---
name: infra-aws-EC2
description: Use when the user invokes %infra-aws-EC2 or asks about deploying a full-stack application (frontend + backend) on AWS using CloudFormation IaC with ECS EC2 launch type — covering CloudFront, S3, ELB, ECS on EC2 with ASG, Aurora, Lambda, ElastiCache, and OpenSearch.
---

# AWS Full-Stack Deployment — ECS EC2 (CloudFormation IaC)

## When to use this skill
Activate when the user types `%infra-aws-EC2` or asks about deploying a full-stack application on AWS using ECS with the **EC2 launch type** (instances managed by an Auto Scaling Group). Use the Fargate skill instead when serverless/cost-optimised compute is preferred.

**This skill is a provider implementation of `%infra-IaC-specification`** — it realises every contract defined in that specification using AWS CloudFormation, ECS EC2, Aurora PostgreSQL, and CodeBuild.

> **Why EC2 over Fargate?** Use EC2 when you need host-level control, custom AMIs, sustained high throughput, or want to pack multiple tasks on a single instance. With Fargate you pay per task; with EC2 you pay per instance regardless of task count — EC2 is more cost-efficient at high, steady-state utilisation.
...
1. **Product-level infrastructure** (deployed once per product per environment):
   ...
   - Shared ECS Cluster + ALB (HTTP :80 with default 404 action) + **ASG + ECS Capacity Provider** → `aws-ecs-infra-stack.yml`
...
2. **Per-service infrastructure** (deployed once per microservice per environment — all stacks under `per-service/`):
   ...
   - IAM roles (task execution + task role + **EC2 instance role + instance profile**) → `aws-iam-stack.yml`
...
Multiple microservices (e.g. `catalog`, `orders`, `payments`) share the same VPC, ECS Cluster, ALB, and EC2 ASG. Each gets its own ALB path-based rule: `/<AppServiceName>/*`.
...
Internet
    ...
    └──► ALB (internet-facing, HTTP :80)
              │
              ├── /catalog/*   → ECS EC2 Service (catalog)
              ├── /orders/*    → ECS EC2 Service (orders)
              └── /payments/*  → ECS EC2 Service (payments)
                       │
              (running on EC2 ASG instances — private subnets)
```
...
|---|---|---|---|
| **EC2 instance type** | `InstanceType` | `t3.medium` ⚠️ JVM needs ≥ 4 GB RAM — smaller instance types will OOM | `t3.large` |
...
| **Task memory** | `TaskMemory` | `768 MB` ⚠️ lower than Fargate because EC2 mode uses `bridge` network — headroom is shared with the OS | `2048 MB` |
...
│   ├── aws-ecs-infra-stack.yml        ← ECS Cluster + ALB + ASG + ECS Capacity Provider (once per product)
│   └── per-service/
│       ├── aws-codebuild-stack.yml    ← ECR repo + CodeBuild project (once per microservice)
│       ├── aws-iam-stack.yml          ← ECS task execution role + task role + EC2 instance role/profile (once per microservice)
...
## Key EC2 vs Fargate Differences
...
| `AssignPublicIp` | N/A (bridge mode) | `DISABLED` |
| Host port | `0` (dynamic) | not set (awsvpc) |
| IAM stack | adds `Ec2InstanceRole` + `Ec2InstanceProfile` | task execution + task role only |
| Infra stack | adds ASG + Launch Template + Capacity Provider | cluster + ALB only |
| `DesiredCount` default | `0` | `0` |
| Compute cost | per EC2 instance (efficient at high utilisation) | per task (efficient at low/variable utilisation) |
...
9. Generate `aws-ecs-service-stack.yml` with `NetworkMode: bridge`, `RequiresCompatibilities: [EC2]`, `TargetType: instance`, and host port `0` (dynamic).
10. Generate `aws-iam-stack.yml` with `Ec2InstanceRole` + `Ec2InstanceProfile` in addition to the task execution and task roles; include a Secrets Manager read policy on the DB secret ARN.
11. Generate `aws-ecs-infra-stack.yml` with an ASG, Launch Template, and ECS Capacity Provider — the EC2 instances must register with the ECS cluster via `UserData`.
12. Generate `aws-rds-aurora-stack.yml` with `ManageMasterUserPassword: true` — never put the database password in plaintext in any CloudFormation parameter or environment variable.
13. Inject `DB_HOST`, `DB_PORT`, `DB_NAME`, and `DB_SECRET_ARN` into the ECS task as environment variables; the application reads the secret at startup via the AWS SDK.
14. Add `sg-rds` to `aws-vpc-stack.yml` — only inbound TCP :5432 from `sg-ecs`.
15. Respond and assist in English unless the user requests another language.
16. Await further instructions and execute them accordingly.
